...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% File Name       : ADVANCE_Compute_Lithosphere_Cell.m
...
%   - FLUX_U     (cm^{-2} s^{-1})    : Geonu flux from urainum
%   - FLUX_Th     (cm^{-2} s^{-1})   : Geonu flux from thorium
...
% % Split cell % %
    ...
    for ii2 = 1 : length(sub_lons)
        ...
        if sub_subcell_size * 1.333 <= subcell_size
            ...
            sig_factor_u238 = sum(sub_sub_volumes .* (sig_response_u238 .* Pee')' , 2); % cell * 1 %
            sig_factor_th232 = sum(sub_sub_volumes .* (sig_response_th232 .* Pee')', 2); % cell * 1 %
            unit_geonu_sig_factor_u238 = sig_factor_u238 ./ (sub_sub_distances .^ 2); % cell * 1 %
            unit_geonu_sig_factor_th232 = sig_factor_th232 ./ (sub_sub_distances .^ 2); % cell * 1 %
            geonu_signal_factor_u238 = geonu_signal_factor_u238 + sum(unit_geonu_sig_factor_u238, 1);  % Unit: m^3/kg %
