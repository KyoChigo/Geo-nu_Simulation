function THICKNESS = Compute_Layer_Thickness(name_model, name_layer, thick_mean, cor, lab_mean, moho_mean)
%%%开始计算
temp = strcmp('Crust1', name_model) + strcmp('Crust2', name_model);
uncertainty = 0.12;
if strcmp(name_model, 'LM') && temp == 1
    lab = Generate_Random_Normal(175000, 75000, 0, cor); % Unit: m
    moho = Generate_Random_Normal(moho_mean, moho_mean * uncertainty , 0, cor); % Unit: m
    THICKNESS = lab - moho; % Unit: m
else
    error = uncertainty * thick_mean; % Unit: m
    THICKNESS = Generate_Random_Normal(thick_mean, error, 0, cor); % Unit: m
end
%%% 小于零的项归零
